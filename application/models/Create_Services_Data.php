...
class Create_Script_Data extends CI_Model 
 ...
 //  $company = $_POST['company'];  
    ...
    function db_chk_name($code) 
        ...
        //echo $this->input->post('txt_branch_name');
        $data = array
            (
       ...
       // print_r($data);
       $this->db->insert('tbl_script', $data);
	...
	//print_r($results);
	  ...
	  foreach ($query->result() as $row) {
            //$data1[] = $row;
			//echo $row->Vendor_Id; 
			$q = $this->db->query("insert into tbl_script_level (script_id,scriprt_name,level_id,lavel_name) values
			(".$branch.",'".$this->input->post('txt_branch_name')."',".$row->id.",'".$row->Name."')");
