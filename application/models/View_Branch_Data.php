<?php
class View_Branch_Data extends CI_Model 
            ...
            public function __construct()
                 {
				  ...
				  function fetch_Domain_data() {
        $u_id=0;
       $v_id = $this->session->userdata('Vendor_Id');      
       $user1 = $this->db->query("select * from tbl_domain" );
      // print_r($user);
         return $user1;
    }
        ...
        $u_id=0;
       $v_id = $this->session->userdata('Vendor_Id');      
       $user1 = $this->db->query("select * from tbl_branch where created_by_userid=". $v_id." and `is_active`=" . $u_id );
      // print_r($user);
         return $user1;
    }
	
	  function fetch_domain_data_to_update($c_id)
                 {
                //echo $c_id;
                            $user = $this->db->query("select * from tbl_domain where id=" . $c_id);
               ...
               // $this->db->close();
                ...
                {       
                 ...
                 function update_branch_data($id) 
                 {
                    ...
                    'url' => $this->input->post('branch_name'),
                 ...
                 //   print_r($data);
                    ...
                    $where1 = "(id='" . $id . "')";
                   $this->db->where($where1);
                    $this->db->update("tbl_domain", $data);
