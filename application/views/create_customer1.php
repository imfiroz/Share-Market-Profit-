		...
		if(code.match(codeformat))
			...
			if(code!="" && code.match(codeformat))
                                         {   
                                            $.ajax
                                            ({	
                                              url: "<?php echo base_url(); ?>"+"Create_Customer/check_name",    
     ...
     var mailformat =/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;	 
		if(mail.match(mailformat))
			...
			if(mail!="" && mail.match(mailformat))
				{   
						$.ajax({	
                                                  url: "<?php echo base_url(); ?>"+"Create_Customer/check_email", 						  
						  method:"POST",
		...
		if(phone.match(phoneformat))
			...
			if(phone!="" && phone.match(phoneformat))
                                         {  
                                            $.ajax
                                            ({	
                                              url: "<?php echo base_url(); ?>"+"Create_Customer/check_phone",                                                     
                                              method:"POST",
            ...
            var txt_phone=$("#phone").val();           
                if(name == "" || txt_email == "" || txt_phone == "")
                    ...
                    { 
                       ...
                       $.ajax({
                           url: "<?php echo base_url(); ?>"+"Create_Customer/customer_save_data",                         
                           type: "POST",
                           data: formData,
                           async: false,
                           //dataType:'JSON',
     ...
     function display_State()
        {
            $.ajax
                ({
                    type: "POST",
                    url: "<?php echo base_url(); ?>"+"Create_Customer/get_state_data",
                    dataType: 'json',                                   
                    success: function (data)
                      {                     
                        $("<option />").val("0")
      ...
      $('#state').on('change',function()
        ...
        if(city){
            $.ajax({
               type:'POST',
               url: "<?php echo base_url(); ?>"+"Create_Customer/get_city",
               data:'city='+city,
                dataType: 'json',        
