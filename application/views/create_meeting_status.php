						...
						<div class="example-box-wrapper">
							<form class="form-horizontal bordered-row"  id="button_submit" name="button_submit">
								<div class="form-group">
									...
									<div class="col-sm-6">
										<input type="text" class="form-control" id="meeting_name" name="meeting_name" placeholder="Enter Meeting Name..." onblur="Validate_meeting()">
										<div id="meeting_verification"></div>					
									</div>
								</div>
                                                                <button class="btn btn-alt btn-hover btn-black" href="#" style="margin-left:450px" type="submit" id="submit_form">
                                                                       <span>Submit</span>
                                                                            <i class="glyph-icon icon-arrow-right">                                                                          
                                                                           </i>
                                                                </button>				
							</form>
		</div>
...
?>
<script>
    function Validate_meeting()
	{
            var code=document.getElementById("meeting_name").value;  
            var helpemail=document.getElementById("meeting_verification");
            var codeformat = /^[a-zA-Z]+$/;                    
                          if(code=="")
				{                                                                  
			...
			if(code!="" && code.match(codeformat))
                                         {   
                                            $.ajax
                                            ({	
                                              url: "<?php echo base_url(); ?>"+"Create_Meeting_Status/check_meeting_verification",    
                                              //url: "../Create_meeting_status/check_meeting_verification/", 
                                              method:"POST",
                                              data: "name="+code,
                                              success: function(data)  
                                                 {	
                                                            // alert(data);
                                                    if(data==0){								
                                                                    $("#meeting_verification").html("<span style='color:red;'>Please enter another  Name its already in use</span>");
                                                                    document.getElementById("submit_form").disabled = true;							
                                                                  }
                                                    else	{
                                                                        $("#meeting_verification").html("<span style='color:green;'> Name Valid</span>");
                        ...
                        if(name == "")
                           {
                               alert("Please insert required fields");
                                   ...
                                   $.ajax({
                                        url: "<?php echo base_url(); ?>"+"Create_Meeting_Status/meeting_data",
                                       type: "POST",
                                       data: formData,
                                       async: false,
                                       dataType:'JSON',
                                                     ...
                                                     {	
                                                        ...
                                                        {	
                                                           alert("Information successfully registered")			  
                                                            window.location ="<?php echo base_url();?>"+"View_Meeting_Status";				
                                                         }				
                                                      },
                                       cache: false,
                                       contentType: false,
