     ...
     function fetch_branch_details()
            {
                ...
                $user_data = $this->View_Branch_Data->fetch_Domain_data();
                 $rows1 = array();
            ...
            {    
                $this->load->model('View_Branch_Data');
                $user_data = $this->View_Branch_Data->fetch_domain_data_to_update($c_id);
                $event = "Branch";
                $address_path = "branch_image";
                $path = $this->View_Branch_Data->db_retrieve_base_path_image($address_path, $event);
            ...
            {       
        $this->load->model('View_Branch_Data');
		$qu=$this->db->query("delete from tbl_domain where id=$c_id");
		echo $this->db->affected_rows();
        //$user_data = $this->View_Branch_Data->fetch_branch_data_to_delete($c_id);                 
    
            }
              function save_update_branch($cust_id)
             {      
                $this->load->model('View_Branch_Data');
                $user_data = $this->View_Branch_Data->update_branch_data($cust_id);
            }
