...
class View_Script extends CI_Controller {
 public function __construct() 
       ...
       public function index()
	{
     ...
     function fetch_branch_details()
            ...
            {    
                $this->load->model('View_Script_Data');
                $user_data = $this->View_Script_Data->fetch_Script_data_to_update($c_id);
                $event = "Branch";
                $address_path = "branch_image";
                $path = $this->View_Script_Data->db_retrieve_base_path_image($address_path, $event);
              ...
              public function get_trading_type() {
        $this->load->model('View_Script_Data');
        $getCountry = $this->View_Script_Data->gettrading_Data();
     ...
     public function get_transaction() {
        $this->load->model('View_Script_Data');
        $getCountry = $this->View_Script_Data->gettransaction_Data();
        foreach ($getCountry->result() as $row) {
            $data1[] = $row;
        }
        echo json_encode($data1);
            ...
            {       
        $this->load->model('View_Script_Data');
         $user_data = $this->View_Script_Data->fetch_script_data_to_delete($c_id);                 
		//echo $this->db->affected_rows();                 
             ...
             {      
                $this->load->model('View_Script_Data');
                $user_data = $this->View_Script_Data->update_branch_data($cust_id);
            }
		...
		$id = $_POST['email'];
            $this->load->model('View_Script_Data');
            $getCountry = $this->View_Script_Data->getcustcount12($id);                   
     ...
     function check_script() 
        {            
           $code = $_POST['hsn_data'];
         $value_trad_typ = $_POST['txt_state'];           
           $this->load->model('View_Script_Data');
            $code_data = $this->View_Script_Data->db_chk_hsn($code,$value_trad_typ);
