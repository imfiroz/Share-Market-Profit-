...
class View_Notification extends CI_Controller {
 public function __construct() 
	...
	{
		$this->load->view('view_notification');
	}
     ...
     function fetch_branch_details()
            {
                ...
                $user_data = $this->View_Notification_Data->fetch_Notification_Data();
                 $rows1 = array();
            ...
            {    
                $this->load->model('View_Notification_Data');
                $user_data = $this->View_Notification_Data->fetch_notification_data_to_update($c_id);
                $event = "Branch";
                $address_path = "branch_image";
                $path = $this->View_Notification_Data->db_retrieve_base_path_image($address_path, $event);
      ...
      $qu=$this->db->query("delete from tbl_notification where id=$c_id");
		echo $this->db->affected_rows();            
             ...
             {      
                $this->load->model('View_Notification_Data');
                $user_data = $this->View_Notification_Data->update_branch_data($cust_id);
            }
