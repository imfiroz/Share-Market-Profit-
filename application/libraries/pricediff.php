...
$value = $_POST['value'];
	switch($update){
			case '1':
			$select = $database->query("SELECT * FROM tbl_stock WHERE stock_grade = '$grade' AND stock_size = '$size' AND stock_cat = '$cat' 
										AND stock_date = '$date' AND stock_round = '$round'");	
			break;
			case '2':
			$select = $database->query("SELECT * FROM tbl_stock WHERE stock_cat = '$cat' AND stock_date = '$date' AND stock_round = '$round'");
			echo "SELECT * FROM tbl_stock WHERE stock_cat = '$cat' AND stock_date = '$date' AND stock_round = '$round'";
			break;
			case '3':
			$select = $database->query("SELECT * FROM tbl_stock WHERE stock_size = '$size' AND stock_date = '$date' AND stock_round = '$round'");	
			break;
			case '4':
			$select = $database->query("SELECT * FROM tbl_stock WHERE stock_date = '$date' AND stock_round = '$round'");	
			break;
		}	
		...
		if($type == 'add'){
			...
			while($row_stock = $database->fetch_array($select)){
				...
				// $query = "UPDATE tbl_stock SET stock_newrate = '$newrate' WHERE stock_id = '$stock_id'";	
				$query = "UPDATE tbl_stock SET stock_newrate = '$newrate' WHERE stock_id = '$stock_id' AND stock_date = '$date' AND stock_time='$time'";	
			...
			while($row_stock = $database->fetch_array($select)){
				$stock_id = $row_stock['stock_id'];
// echo $row_stock['stock_newrate']."<br/>";
// echo $value."<br/>";
